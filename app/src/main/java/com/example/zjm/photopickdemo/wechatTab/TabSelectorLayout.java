...
/**
 * Created by zjm on 16-9-26.
 */

public class TabSelectorLayout extends ViewGroup {
    private static final String TAG = TabSelectorLayout.class.getSimpleName();
        ...
        try {
            ...
            for (int i = 0; i < count; i++) {
                ...
                if (index == R.styleable.TabSelectorLayout_normalTextColor) {
                    ...
                    drawablePadding = array.getDimensionPixelOffset(index, 10);
                } else if (index == R.styleable.TabSelectorLayout_android_textSize) {
                    textSize = array.getDimensionPixelSize(index,
                            (int) TypedValue.applyDimension(TypedValue.COMPLEX_UNIT_DIP, textSize, getResources().getDisplayMetrics()));
                }
            }
    ...
    protected void onMeasure(int widthMeasureSpec, int heightMeasureSpec) {
        ...
        final int count = getChildCount();
        int childWith = (width - getPaddingLeft() - getPaddingRight()) / count;
        int maxChildHeight = 0;
        for (int i = 0; i < count; i++) {
            final View child = getChildAt(i);
            if (child.getVisibility() != GONE) {
                child.measure(MeasureSpec.makeMeasureSpec(childWith, MeasureSpec.EXACTLY), MeasureSpec.makeMeasureSpec(height, MeasureSpec.AT_MOST));
